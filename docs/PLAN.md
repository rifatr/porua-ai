...
| Celery / Redis job queue | One `docker compose up` is better than an extra service. **Built differently from this plan, which said background tasks with a status column.** Processing is synchronous, inside the request, because extraction is where bad input is *discovered* — a background task turns every typed rejection into a status column the client has to poll and then interpret. So there is no status column, and each bad file is an HTTP status instead; reasoning in [`DECISIONS.md`](DECISIONS.md#a-bad-upload-is-an-http-status-not-a-status-field). The parse-and-chunk step already runs in a worker thread and moves to a real worker unchanged when the 20 MB cap stops being the right trade. |
...
| More document formats | `.doc` (old binary Word — needs a converter, not a parser), `.odt`, `.rtf`, `.txt`, `.md`, `.epub`. None of this needs OCR — it needs more extractors behind the same `Page` interface, which is why extraction is one function per format returning a shared shape. Left out because three formats cover what the brief names, and each new one is a new set of bad-file cases to get right rather than a line in a dispatch table. |
| The parts of a file we hold and do not read | Word headers, footers, text boxes, footnotes, endnotes and comments; PowerPoint SmartArt and chart data; images and image-rendered formulas in any format. Every one is **silent** — no error, just a document that lacks what the student can see, first noticed when the tutor says the material does not cover something it visibly does. Word headers and footers are worth doing first: a cover page's date and course code live there, and a real progress report uploaded during testing had no findable submission date for exactly this reason. `python-docx` walks the body only, so the fix is reading `word/header1.xml` and `word/footnotes.xml` directly — a morning's work. Text inside **grouped** PowerPoint shapes was in this list until testing found it: a group is a shape holding shapes and has no text of its own, so iterating the top level dropped whole labelled diagrams. Fixed, with a test for nested groups. |
| Semantic retrieval | Search is lexical, and the boundary is sharper than "worse recall". Asked for a submission date, the tutor correctly reported finding none while slide 1 read `AUGUST 2026` — **a date does not contain the word "date"**. Same for "how plants eat" against "photosynthesis". No ranking change reaches these; they are outside what word matching can do. Embeddings alongside the `tsvector`, results merged, and the interface is already narrow enough for it — one service function, two callers. Needs a second model, which is the only reason it waits. |
| Grounded-answer bookkeeping | A question answered badly once tends to be answered badly again: the earlier reply is in the room's context, and the model recaps itself instead of searching. Prompting helps and does not win against three prior answers. The fix is a column recording whether a turn was grounded in a document, and dropping ungrounded answers from context when the same topic returns — so the tutor's own guess never competes with the student's handout. |
...
| Taking the quiz | `quiz_builder` produces a quiz and stops there. Nothing submits answers, nothing scores, and there is no attempt table — so `is_correct` and `explanation` ship inside the questions, where a student who can read the response can read the answer key. That is not a leak today, because the only consumer is a person inspecting what was generated, and for *them* `is_correct` is load-bearing: it is how you see that exactly one option is correct and that the shuffle moved it off the model's slot. It is a shape with no room for the feature it implies. Grading needs three things, and only the third is real work: the run response drops `is_correct` and `explanation`; `POST /quiz-runs/{turn_id}/answers` takes `[{question_id, choice_id}]` and returns per-question correctness with the explanations attached; and a `quiz_attempts` table records what was answered and when, which is the point where a quiz stops being a document and starts being data — mastery per concept becomes a query, `query_study_history` can report what the student is actually weak at rather than what they mentioned, and the rejected "progress tracker" tool in §8 becomes buildable for the reason given there. `GET /turns/{id}/skill-run` keeps everything either way: it already shows raw prompts and raw model replies, so the answer key is the least sensitive thing in it. |
