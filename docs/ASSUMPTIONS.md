...
| **Provider structured output** | Gemini offers `response_schema`, which would largely remove parse failures. | Used, after first being rejected. `response_schema` is a field on `LLMRequest`, not something the Gemini client knows about; the parse, schema and content layers all still run. | Rejected initially as provider-specific, which was the wrong objection — putting the schema on the request keeps `LLMClient` a Protocol, because a provider without structured output ignores the field. What changed the decision was a real failure: a turn where the model answered in prose, restated the whole answer as JSON, and ran out of budget partway through the restatement. Constrained decoding makes that draft impossible rather than discouraged. The original note already said "in production I would use it *and* keep the content layer" — this is that, and the content layer is untouched. |
...
| **Tools vs. structured output** | Both are wanted; the provider allows only one per call. | Verified with three real API calls before designing: Gemini returns `400 "Function calling with a response mime type: 'application/json' is unsupported"`. So a call either gathers with tools or answers under a schema, and `LLMRequest.__post_init__` refuses the combination. | Worth stating as a consequence rather than a workaround: constrained decoding is unavailable exactly while the model can call tools, so during a tool-using turn the parse/schema/content pipeline is the *only* thing between model output and the student. `response_schema` narrowed where that pipeline is the sole defence; it did not replace it. |
| **What the history tool takes** | The endpoint takes a date range. Should the tool? | No — the tool takes `days_back`, the endpoint keeps dates. | The model does not reliably know today's date, so asking it for `from_date` invites a confident wrong answer. A wrong range returns an empty result that reads as "you studied nothing", which is worse than an error. `days_back=7` needs no calendar knowledge, and the arithmetic happens where today's date is a fact. |
| **Running out of tool budget** | Fail the turn, or carry on? | Stop *offering* the tools. The model must then answer with what it gathered. | A student is waiting synchronously. An answer built on partial information beats an error, and the loop still terminates because the tools are simply no longer available to ask for. Failing would be the easier code and the worse product. |
| **Where study history lives** | It is both an acceptance item and their example tool. | One service; the endpoint and the tool both call it. S7 therefore landed inside S4. | Written twice they would drift, and "what did I study" would depend on who asked. Building both at once also cost almost nothing — the query is the expensive part — and removed a slice from the final day. |
